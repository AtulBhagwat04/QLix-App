...
class QuizRepositoryImpl implements QuizRepository {
  ...
  @override
  Future<Map<String, dynamic>> activateQuizQuestion(
    String sessionId,
    String pollId,
    int timeLimit,
  ) async {
    final response = await _apiClient.dio.post(
      '/quiz/activate',
      data: {'sessionId': sessionId, 'pollId': pollId, 'timeLimit': timeLimit},
    );
    ...
    throw Exception(
      response.data?['message'] ?? 'Failed to activate quiz question',
    );
  }
  ...
  Future<Map<String, dynamic>> submitQuizAnswer(
    ...
    throw Exception(
      response.data?['message'] ?? 'Failed to submit quiz answer',
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getLeaderboard(String sessionId) async {
    final response = await _apiClient.dio.get(
      '/quiz/session/$sessionId/leaderboard',
    );
    if (response.statusCode == 200 && response.data != null) {
